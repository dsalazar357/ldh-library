...
import { deleteRitualAction } from "@/app/actions/rituals";

interface RitualDeleteButtonProps {
  ritualId: number;
  ritualTitle: string;
  dict: {
    deleteRitual: string;
...
export default function RitualDeleteButton({
  ritualId,
  ritualTitle,
  dict,
  commonDict,
}: RitualDeleteButtonProps) {
  ...
  if (state?.success) {
    return (
      <span className="text-xs text-green-700 font-medium">{state.success}</span>
    );
  }
